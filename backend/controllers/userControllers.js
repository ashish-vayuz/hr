...
//get all reddem request by admin
const coinRedeemRequestList = asyncHandler(async (req, res) => {
  const getAll = await UserRedem.find().populate("user").sort({ _id: -1 });
  if (getAll) {
...
const coinRedeemRequestUpdate = asyncHandler(async (req, res) => {
  // console.log(req.body, "working,,,");
  const findRequest = await UserRedem.findById(req.body.id);
  if (findRequest) {
    findRequest.status = req.body.status;
    const restData = await findRequest.save();
    if (restData) {
